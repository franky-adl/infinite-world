...
export default class ChunkHelper {
    ...
    setArea() {
        ...
        this.area.material.color.multiplyScalar(
            (this.chunkState.depth + 1) / this.state.chunks.maxDepth,
        );
    ...
    setId() {
        ...
        this.id.display({
            ...
            position: new THREE.Vector3(
                0,
                (this.state.chunks.maxDepth - this.chunkState.depth) * 10,
                0,
            ),
        });
    ...
    setNeighboursIds() {
        ...
        if (!this.neighboursIdVisible) return;

        if (this.chunkState.neighbours.size === 0) return;
        ...
        const size =
            (this.state.chunks.maxDepth - this.chunkState.depth + 1) * 6;
        ...
        this.neighboursIds.display({
            ...
            color: "#00bfff",
            size: size,
            position: new THREE.Vector3(0, y, -this.chunkState.quarterSize),
        });
        ...
        this.neighboursIds.display({
            ...
            color: "#00bfff",
            size: size,
            position: new THREE.Vector3(this.chunkState.quarterSize, y, 0),
        });
