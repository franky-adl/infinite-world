...
const linearStep = (edgeMin, edgeMax, value) => {
    return Math.max(
        0.0,
        Math.min(1.0, (value - edgeMin) / (edgeMax - edgeMin)),
    );
};
...
onmessage = function (event) {
    ...
    const waterLevel = event.data.waterLevel;
     ...
     * Elevation
     */
    const overflowElevations = new Float32Array(
        (segments + 1) * (segments + 1),
    ); // Bigger to calculate normals more accurately
    ...
    const positions = new Float32Array(
        segments * segments * 3 + skirtCount * 3,
    );
    ...
    for (let iZ = 0; iZ < segments; iZ++) {
        for (let iX = 0; iX < segments; iX++) {
            ...
            const neighbourZElevation =
                overflowElevations[iOverflowStride + segments + 1];
    ...
    const indices = new (indicesCount < 65535 ? Uint16Array : Uint32Array)(
        indicesCount * 6 + subdivisions * 4 * 6 * 4,
    );
